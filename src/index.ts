    ...
    for (let m = 0; m < self.config.showMonths; m++) {
      ...
      for (let i = 0, l = month.children.length; i < l; i++) {
        ...
        if (elem !== undefined) {
          ...
          let diffDays = Math.round(Math.abs((hoverDate - self.selectedDates[0])/(oneDay)));
          let hoverNightWording = 'Nächte'
          let hoverDayWording = 'Tage'
          if (diffDays === 1) {
            hoverNightWording = 'Nacht'
            hoverDayWording = 'Tag'
          }
          switch (self.config.locale) {
            case 'en':
              if (diffDays === 1) {
                hoverNightWording = 'night'
                hoverDayWording = 'day'
              } else {
                hoverNightWording = 'nights'
                hoverDayWording = 'days'
              }
              break
          }
          if (document.getElementsByClassName('c-navigation__item--packages').length > 0) {
            if (document.getElementsByClassName('c-navigation__item--packages')[0].classList.contains('c-navigation__item--selected')) {
              elem.dataset.tooltip = diffDays + ' ' + hoverDayWording
            } else {
              elem.dataset.tooltip = diffDays + ' ' + hoverNightWording
            }
          } else {
            elem.dataset.tooltip = diffDays + ' ' + hoverNightWording
          }
          elem.setAttribute('data-tooltip-pos', 'up')
